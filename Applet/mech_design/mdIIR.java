...
public class mdIIR {
	 ...
	 * 
	 * Given a game object, this function will return a type profile when all
	 * the players are playing their true types
	 */
	 ...
	 * function essentially transforms strategies in SCF to indexes in utility
	 * matrix
	 */
	 ...
	 */
	static void find_all_utils(mdGameObject mdo, int player, int cur_type,
			HashMap<String, Integer> poss) {
	 ...
	 * This function essentially computes the utility when player is not telling
	 * the truth
	 */
	static String get_tmp_util_string(String cur_strat, int player,
			mdGameObject mdo, int alt) {
		int key = mdo.scf_matix.get(cur_strat);
		String[] data = cur_strat.split(" ");
		String result = "";
	 ...
	 */
	static float expected_util(mdGameObject mdo, int player, int cur_type,
			String tmp_strat, String cur_strat) {
		float retval = 0;
		int j = 0;
		...
		for (Iterator I = keys.iterator(); I.hasNext(); j++) {
			...
			String tp_Ustart = get_tmp_util_string(cur_strat, player, mdo, poss
					.get(tp_strat));
	...
	/**
	 * int isIIR(String , int , mdGameObject )
	 * 
	 * Given the actual strategies, a player and mdGameObject this fucntion will
	 * compute whether the SCF is IIR for the input player
	 */
	static int isIIR(String actualset, int player, mdGameObject mdo) {

		String[] data = actualset.split(" ");
		int cur_type = Integer.parseInt(data[player]);
		// Entering the probalilites for players

		float cur_util_int = expected_util(mdo, player, cur_type, actualset,
				actualset);
	...
	/**
	 * String find_IIR( mdGameObject )
	 * 
	 * This is the main IIR function. This will compute whether a given SCF is
	 * IIR or not for each of the players. Given the mdGameObject this fucntion
	 * will compute whether the SCF is IIR for the all the players
	 */

	static String find_IIR(mdGameObject mdo) {
		String actualset;
		actualset = getCurset(mdo);
		StringBuffer res = new StringBuffer();

		for (int player = 0; player < mdo.nPlayer; player++) {
			if (isIIR(actualset, player, mdo) == 1) {
				res.append("f() is IIR for player" + (player + 1) + "\n");
			} else {
				res.append("f() is Not IIR for player" + (player + 1) + "\n");
			}
